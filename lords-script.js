...
// Video Preloader Integration - Updated to play only once per session
document.addEventListener('DOMContentLoaded', function() {
    ...
    const hasPlayedVideo = sessionStorage.getItem('hasPlayedIntroVideo');
    
    // If video has already been played, skip it entirely
    ...
    if (!videoPreloader || !video) {
        // If no video preloader, start animations immediately
        console.log('No video preloader found, starting animations directly');
        startHeroAnimations();
        return;
    }
    ...
    function startPreloader() {
        ...
        // End preloader when video ends
        video.addEventListener('ended', function() {
            endPreloader();
        });
        
        // Auto-end after 35 seconds as fallback
        setTimeout(() => {
            endPreloader();
        }, 35000);
    }
    ...
    function endPreloader() {
        ...
        videoPreloader.classList.add('fade-out');
        video.pause();
    ...
    function startMainPageAnimations() {
        // Call your existing startHeroAnimations function
        if (typeof startHeroAnimations === 'function') {
            startHeroAnimations();
        }
        
        // Trigger any other animations that should start after preloader
        // If you have any specific animations that need to be triggered
    }
    
    // Fallback: If video fails to load, end preloader after a timeout
    video.addEventListener('error', function() {
        console.log('Video failed to load, starting animations anyway');
        setTimeout(endPreloader, 1000);
    });
    
    // Optional: Start with muted video if you want it to autoplay (some browsers allow this)
    // video.play().catch(() => {
